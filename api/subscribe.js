...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (upstream.ok) {
      // Kick off the n8n welcome series — only for genuinely NEW subscribers
      // (already-subscribed hits the 409 branch below and is skipped).
      // Failure here must never break the subscription itself.
      const hook = process.env.N8N_WELCOME_WEBHOOK;
      if (hook) {
        try {
          await fetch(hook, {
            method: 'POST',
        ...
        } catch (e) { /* welcome series is best-effort */ }
      }
      res.status(200).json({ ok: true });
    } else {
